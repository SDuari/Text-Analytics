#############################################################################################
# This script is created for hands-on sessions on TEXT Analytics for the FDP organised by   #
# Geethanjali College of Engineering and Technology, Hyderabad (Date: 29th November, 2017). #
#                                                                                           #
...
doc<-c(texts) # bind the characters together
corp = Corpus(VectorSource(doc)) # create the corpus


# Another way to create a corpus (using DirSource)
...
ConstructGraph <- function(win_size) { 
  word_graph <- make_empty_graph(n=0, directed=FALSE)    # creates an empty graph
  i <- 1
...
# TEXTRANK and k-core retention

# Set window size
win_size = 4
# Or prompt the user to pass it
...
text_graph <- ConstructGraph(win_size)
...
plot.igraph(text_graph, edge.label = E(text_graph)$weight)
# or use tkplot(g)
